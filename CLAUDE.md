...
- `src/lib/liquipedia/` + `src/components/liquipedia/` + `scripts/refresh-liquipedia.mjs` — the **Liquipedia tab**: scene-wide Melee history (majors, SSBMRank editions, career winnings), the one view that shows nothing about the user's own replays. `data.ts` is a **generated bundled snapshot**, not a runtime fetch: the app has no backend and the CSP allows self + Supabase only, so the dataset ships in the (lazy) chunk and works offline. A monthly GitHub Action (`.github/workflows/liquipedia-refresh.yml`) runs the refresh script, which is **append-only** — it adds majors dated after the snapshot and ranking editions not yet present, and never rewrites existing rows, so a parser miss costs an update rather than the dataset. It exits 2 when Liquipedia rate-limits it, which the workflow treats as "nothing to do".
...
Categorical series colors (the Liquipedia character charts) come from `CHAR_COLORS` in `src/lib/liquipedia/chars.ts`: a fixed 7-hue order validated for colorblind separation against the `--panel` surface, with everything past it folded into a single `Other` grey — never generate an eighth hue. `CHAR_STACK_ORDER` is that validated adjacency; stacked charts must keep it. Ordinal splits (major vs supermajor) use two steps of one hue instead, and pair them with a legend plus a tooltip listing the underlying rows, since the dim step sits under 3:1 on this surface.
...
node scripts/refresh-liquipedia.mjs   # top up the bundled scene dataset (monthly Action runs this)
```
