...
3. **Frames are discarded.** Custom frame-level metrics (wavedash quality, ledgedashes) are a v2 feature behind an explicit opt-in re-parse — don't retain frames to make a feature easier.
   - When a new per-game stat is added, bump the Dexie version and **clear the games table** (owner-approved). A one-time full re-parse beats mixed-schema rows silently understating averages; don't build per-row schema versioning or lazy re-parse machinery for this.
